...
export class Boundary {
	...
	static width = 16
	static height = 16
	constructor({ x, y, action = undefined, width, height, teleport }: BoundaryInterface) {
		this.mapPosition = new Vector2(x * GameSettings.scale + MapItems.GameMap.offsetX * GameSettings.scale, y * GameSettings.scale + MapItems.GameMap.offsetY * GameSettings.scale)
		this.action = action
	...
	collide(player: Player, background: MapItems.GameMap, px: number, py: number, pw: number, ph: number, collisions: (number | number[])[]) {
		...
		if (px < this.mapPosition.x + this.width - 12 * GameSettings.scale && px + pw > this.mapPosition.x + 12 * GameSettings.scale && py < this.mapPosition.y + this.height - 18 * GameSettings.scale && py + ph > this.mapPosition.y + 9 * GameSettings.scale) {
			if (this.action !== 1) {
				...
				if (this.teleport) Action.execute(background, this.action!, this.teleport, collisions)
			}
			return true
		}
		return false
...
export class Collisions {
	...
	static createBoundary(GameMap: MapItems.GameMap, row: number, col: number, cell: number | number[]) {
	...
	static col(GameMap: MapItems.GameMap, collisions: (number | number[])[]) {
		Collisions.items = []
...
export class Action {
	...
	static handlers: { [key: number]: (GameMap: any, collisions: (number | number[])[]) => void } = {
		2: (GameMap, collisions) => {
			Collisions.col(GameMap, collisions)
		},
		3: (GameMap, collisions) => {
			Collisions.col(GameMap, collisions)
		},
		4: (GameMap, collisions) => {
			Collisions.col(GameMap, collisions)
		},
		5: (GameMap, collisions) => {
			Collisions.col(GameMap, collisions)
		},
	}
	...
	static processObject(GameMap: MapItems.GameMap, row: number, col: number, cell: number | number[]) {
		if (Array.isArray(cell)) {
			switch (cell[0]) {
				...
				case 2760:
					...
					return [...bridge7.boundaries, new Boundary({ x: row * 16 + 3, y: col * 16, width: 4, height: 16 })]
				case 2761:
				...
				case 2765:
					...
					return [...bridge9.boundaries, new Boundary({ x: row * 16 + 10, y: col * 16, width: 3, height: 16 })]
				case 2749:
				...
				case 24:
					return [new Boundary({ x: row * 16 + 9, y: col * 16 + 9, width: 7, height: 7 })]
				case 25:
					return [new Boundary({ x: row * 16, y: col * 16 + 9, width: 16, height: 4 })]
				case 28:
					return [new Boundary({ x: row * 16, y: col * 16 + 9, width: 7, height: 7 })]
				case 29:
					return [new Boundary({ x: row * 16 + 3, y: col * 16 + 3, width: 13, height: 9 })]
				case 30:
					return [new Boundary({ x: row * 16, y: col * 16 + 3, width: 13, height: 9 })]
				case 46:
					return [new Boundary({ x: row * 16 + 9, y: col * 16, width: 4, height: 16 })]
				case 50:
					return [new Boundary({ x: row * 16 + 3, y: col * 16, width: 4, height: 16 })]
				case 51:
					return [new Boundary({ x: row * 16 + 3, y: col * 16 + 9, width: 13, height: 4 }), new Boundary({ x: row * 16 + 3, y: col * 16, width: 4, height: 9 })]
				case 52:
					return [new Boundary({ x: row * 16, y: col * 16 + 9, width: 13, height: 4 }), new Boundary({ x: row * 16 + 9, y: col * 16, width: 4, height: 9 })]
				case 90:
					return [new Boundary({ x: row * 16 + 9, y: col * 16, width: 7, height: 12 })]
				case 91:
					return [new Boundary({ x: row * 16, y: col * 16 + 3, width: 16, height: 9 })]
				case 94:
					return [new Boundary({ x: row * 16, y: col * 16, width: 7, height: 12 })]
				case 95:
					return [new Boundary({ x: row * 16, y: col * 16 + 3, width: 64, height: 9 }), new Boundary({ x: row * 16 + 10, y: col * 16 - 13, width: 45, height: 16 })]
			}
		} else if (cell !== 0) {
