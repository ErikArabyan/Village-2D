import { Sprite, Menu, Animation, Text, Sound, GameSettings } from './gamelib.ts'
...
export class Player extends Animation {
	...
	constructor() {
		super(['players/Player1.png', 'players/Player_Actions.png'], 35, 32, 32, 32, 32, Player.initialPosX, Player.initialPosY, 60 * GameSettings.scale, 8)
...
export class Settings extends Menu {
	...
	constructor(x: number, y: number, width: number, height: number) {
		...
		this.help = [new Sprite('Items/enter.png', ((x + width) / 2 - 86) / GameSettings.scale, ((y + height) / 2 - 100) / GameSettings.scale), new Sprite('Items/arrows.png', ((x + width) / 2 - 10) / GameSettings.scale, ((y + height) / 2 - 146) / GameSettings.scale)]
		this.items = [new Option('Play Music (Enter)', (x + width) / 2 - 86, (y + height) / 2 - 32, () => this.music.play(), 0), new Option('Pause Music (Enter)', (x + width) / 2 - 86, (y + height) / 2, () => this.music.pause(), 1), new Option('Volume Change (<-- -->)', (x + width) / 2 - 86, (y + height) / 2 + 32, (keys: Record<string, boolean>) => this.volume(keys), 2)]
		this.activeItem = 0
	}
